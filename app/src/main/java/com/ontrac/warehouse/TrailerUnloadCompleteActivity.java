...
public class TrailerUnloadCompleteActivity extends SharedScannerActivity implements APIsEventListener {
    private static final String Title =  "Unload Trailer Complete {1} - {0}";
    ...
    @BindView(R.id.uxTrailerStatus)
    Spinner uxTrailerStatus;

    @BindView(R.id.uxButtonPost)
    Button uxButtonPost;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_trailer_unload_complete);
        ...
        if (!changingOrientation) {
            ...
            uxTrailerStatus.setEnabled(false);
            uxButtonPost.setEnabled(false);
...
//                    UpdateAdditionalData();
            ...
            case "uxTrailer" :{
                this.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        uxTrailerStatus.setEnabled(true);
                        uxTrailerStatus.requestFocus();
                        uxButtonPost.setEnabled(true);
                    }
                });
                trailer = data;
                break;
    ...
    @OnClick(R.id.uxButtonPost)
    protected void uxcmdPost(){
        com.ontrac.warehouse.Utilities.Notifications.Vibrate();
        String status =  uxTrailerStatus.getSelectedItem().toString();
        if (Strings.IsNullOrWhiteSpace(status)) {
            General.Alert(this, "Trailer Status Error", "A status selection is required.");
        }
        else {

            startLookupDialog("Prcoessing...", "Attempting to complete trailer.");
            ...
            if (status.compareTo("Empty") == 0)
                trailerProcessRequest.IsEmpty = true;
            else
                trailerProcessRequest.IsEmpty = false;
            trailerProcessRequest.ProcessType = "COMPLETE";

            CompleteTrailer(trailerProcessRequest);
        }
    }

    protected void CompleteTrailer(APIs.TrailerProcessRequestObject trailerProcessRequest)
    {
        ...
        switch (type) {
            case APIs.Type.TrailerProcess:
                if (responseCode == 200)
                {
                    ...
                    General.Alert(this,"Trailer Complete Message", trailerProcessResults.message ,AfterSuccessAlertCallable());
                }
                else
                    this.runOnUiThread(new Runnable() {
                        @Override
